...
## javadoc tags
`smart-doc`的设计初衷是利用`JavaDoc`文档注释以减少注解所带来的侵入性。
因此，每当`smart-doc`增加新功能时，首先会考虑的是`JavaDoc`的原生标签。接下来，
将介绍`smart-doc`所使用的一些`JavaDoc`注释标签。
...
针对一些特殊场景，由于`JavaDoc`的标签较为有限，可能无法完全表达用户的使用需求。因此，`smart-doc`官方在必要时谨慎地引入了一些自定义标签，
以更好地满足文档化需求，同时确保与`JavaDoc`的兼容性和一致性。
使用介绍如下：
